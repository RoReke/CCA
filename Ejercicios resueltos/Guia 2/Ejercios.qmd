...
```{r}
#data <- read.csv2("data/GSPC.csv", sep = ",")
```
...
stats_data <- data %>%
  summarize(
    ...
    max = apply(., max),
    mean = apply(., 2, mean),
   # q1 = apply(., 2, quantile, 0.25),
    median = apply(., 2, median),
    q3 = apply(., 2, quantile, 0.75),
    range = apply(., 2, range),
    sd = apply(., 2, sd),
    iqr = apply(., 2, IQR),
    mad = apply(., 2, mad),
    cv = apply(., 2, function(x) sd(x) / mean(x))
  )

# Agrega los nombres de las columnas
colnames(stats_data) <- c("min", "max", "mean", "q1", "median", "q3", "range", "sd", "iqr", "mad", "cv")
